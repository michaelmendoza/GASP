...
def run_gasp(I: npt.NDArray, An: npt.NDArray, method :str = "affine", has_coils: bool = False):
    ...
    if has_coils:
        # Reshape the data to be in the form [Height x Width x Coils, PCs x TRs]
        height, width, coils = I.shape[:3]
        I = I.reshape(I.shape[0], I.shape[1], I.shape[2], -1)    # Collapse all but first 3 dimensions
        I = I.reshape((-1, I.shape[-1]))                         # Collapse all dimensions last dimension
        npcs = I.shape[-1]
        data_shape = (height, width, coils)
    else:   
...
def train_gasp(I: npt.NDArray, D: npt.NDArray, method: str = "affine", has_coils: bool = False):
    ...
    method (str, optional): Method used to compute the GASP model solution.
        Must be one of {"affine", "linear", "lev-mar", "lev-mar-quad"}.
    has_coils (bool, optional): Whether the data has coils data in 3rd dimension.
...
def process_data_for_gasp_old(M, D=None, useMask=False, useCalibration=False, clines=2):
